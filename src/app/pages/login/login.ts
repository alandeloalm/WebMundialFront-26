...
@Component({
  selector: 'app-login',
  standalone: true,
  imports: [CommonModule, FormsModule, RouterLink, LucideAngularModule, PaisSelector, GeneroSelector],
...
export class Login implements AfterViewInit {
  ...
  ngAfterViewInit() {
    try {
      ...
      console.warn('Google SDK no disponible:', e);
    }
  }
  ...
  toggleMode() {
    ...
    if (this.paisSelector) this.paisSelector.clear();
  }
  ...
  private handleLogin() {
    ...
    this.auth.login(this.formData.email, this.formData.password).subscribe({
      ...
      error: (err) => {
        ...
        if (err.status === 0) this.errorMsg.set('Sin conexión. Verifica tu internet.');
        else if (err.status === 403) this.errorMsg.set(msg || 'Tu cuenta ha sido desactivada.');
        else if (err.status === 500) this.errorMsg.set('Error en el servidor. Intenta más tarde.');
        else this.errorMsg.set(msg || 'Correo o contraseña incorrectos.');
  ...
  private handleRegistro() {
    ...
    this.auth.registro({
      ...
      error: (err) => {
        const msg = err.error?.error;
        if (err.status === 0) this.errorMsg.set('Sin conexión. Verifica tu internet.');
        else if (err.status === 409) this.errorMsg.set(msg || 'El correo ya está registrado.');
        else if (err.status === 500) this.errorMsg.set('Error en el servidor. Intenta más tarde.');
        else this.errorMsg.set(msg || 'Error al crear la cuenta.');
        this.isLoading.set(false);
      }
    });
  }
  ...
  handleGoogleLogin() {
    ...
    try {
      ...
      this.errorMsg.set('Google no disponible en este momento.');
    }
  }
  ...
  private handleGoogleCallback(idToken: string) {
    ...
    this.auth.loginConGoogle(idToken).subscribe({
      ...
      error: (err) => {
        ...
        else this.errorMsg.set(msg || 'No se pudo iniciar sesión con Google.');
        this.isLoading.set(false);
      }
    });
  }
